...
Replace contents of `pyproject.toml` (note: the `[build-system]`, `[tool.uv]`, and `[tool.setuptools.packages.find]` blocks are required so `[project.scripts]` actually installs the `lotto` console script in Step 5):
...
Also create a minimal stub `lottoml/cli.py` so the entry point resolves:

```python
"""lotto CLI entrypoint (commands added incrementally in later tasks)."""
import click
